...
pub static RAYDIUM_DECODER: Lazy<Arc<dyn ProgramDecoderMatcher>> = Lazy::new(|| {
    Arc::new(RaydiumProgramDecoderMatcher{}) as Arc<dyn ProgramDecoderMatcher>
});
...
impl ProgramDecoder for Programs {
    fn decode(&self) -> anyhow::Result<crate::types::DecodedInstruction> {
        match self {
            Self::CpmmV1(decoder) => {
                return decoder.decode().with_context(|| "failed to decde Raydium::CpmmV1");
            }
        }
        Err(anyhow!("TODO"))
    }
